...
property savePath : ((path to scripts folder) as text) & "Applications:" & targetEditor & ":CodeWrapper Scripts:" -- default save location
...
on initialize() -- reset globals and get options
	set {selectionOption, compileOption, saveOption, prefixOption, authorOption, wrapperComment, editor} ¬
...
to getOptions() -- get a few option choices for the current run
	tell |+|'s NSAlert's alloc()'s init() to try
		...
		set results to its runModal as integer
		if results is 1001 then my applyAccessoryValues(accessory)
		if results is 1002 then error "User cancelled." number -128
		if results is 1003 then
...
to makeAccessory() -- UI items for the initial options dialog
	...
	tell (|+|'s NSBox's alloc()'s initWithFrame:{{0, 0}, {410, 177}})
		...
		its addSubview:(my (makeCheckbox at {12, 120} given title:"Compile target document after inserting text", tag:222, state:compileOption))
		set checkbox to (my (makeCheckbox at {12, 100} given title:"Auto save wrapper script to \"" & saveName & "\" folder", tag:333, state:saveOption))

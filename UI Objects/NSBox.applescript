...
to makeBox at origin given dimensions:dimensions : {10, 20}, boxType:boxType : missing value, title:title : "", titlePosition:titlePosition : 0, titleFont:titleFont : missing value, borderType:borderType : missing value, borderColor:borderColor : missing value, borderWidth:borderWidth : missing value, cornerRadius:cornerRadius : missing value, fillColor:fillColor : missing value
	tell (current application's NSBox's alloc's initWithFrame:{origin, dimensions})
		if boxType is not missing value then its setBoxType:boxType
		...
		if borderType is not missing value then its setBorderType:borderType

...
to makeBox at origin given dimensions:dimensions : {10, 20}, title:title : "", titlePosition:titlePosition : 0, titleFont:titleFont : missing value, boxType:boxType : missing value, borderColor:borderColor : missing value, borderWidth:borderWidth : missing value, cornerRadius:cornerRadius : missing value, fillColor:fillColor : missing value
	tell (current application's NSBox's alloc's initWithFrame:{origin, dimensions})
		...
		if boxType is not missing value then
			if boxType is 4 then -- NSBoxCustom
				...
				its setBoxType:boxType -- NSBoxSeparator (2) or NSBoxCustom (4)
			end if
		end if
		return it
	end tell

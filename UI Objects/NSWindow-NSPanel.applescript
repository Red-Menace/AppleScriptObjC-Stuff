...
to makeWindow at origin given dimensions:dimensions : {400, 200}, styleMask:styleMask : 7, title:title : "", panel:panel : false, floats:floats : false, hasShadow:hasShadow : true, minimumSize:minimumSize : missing value, maximumSize:maximumSize : missing value, backgroundColor:backgroundColor : missing value
	if panel is true then
		set theClass to current application's NSPanel
	else
		set theClass to current application's NSWindow
	end if
	tell (theClass's alloc()'s initWithContentRect:{{0, 0}, dimensions} styleMask:styleMask backing:2 defer:true)
		...
		if minimumSize is not missing value then its setMinSize:minimumSize
		if maximumSize is not missing value then its setMaxSize:maximumSize
